...
<#-- 输出分页表单 -->
    ...
    <#-- 把请求中的所有参数当作隐藏表单域(无法解决一个参数对应多个值的情况)-->
        ...
        <#if (page != 1)>
        <li><a href="javascript:void(0)" onclick="jumpPage_${pageid!''}(${page - 1})"><</a>
        </li>
        </#if>
        ...
        <#list start..end as i>
        <li <#if i==page>class="current"</#if>>
            <a href="javascript:void(0)" onclick="jumpPage_${pageid!''}(${i})">${i}</a>
        </li>
        </#list>
        <#if (page != pageCount)>
        <li><a href="javascript:void(0)" onclick="jumpPage_${pageid!''}(${page + 1})">></a></li>
        </#if>
    </#if>
</#macro>